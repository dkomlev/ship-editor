...
- app.json — мир/отрисовка/HUD/управление/коллизии/логирование/отладка + ship_config_path (см. «U2 — ТЗ AppConfig v0.5.3»)
- ship.json — все ТТХ корабля (масса, геометрия, спрайт, движители, g‑лимиты, ассисты) (см. «U2 — ТЗ ShipConfig v0.5.3»)
...
- Пространство: тороидальное поле bounds_rect с wrap‑around; телеметрия ведётся в СИ (м, м/с, рад, кг).
...
- **Автопилот (Random):** R — включить/выключить; по умолчанию активен сразу после старта приложения. Любой ручной ввод (клавиатура/тач) или повторное нажатие R отключает режим. Режим автопилота не изменяет состояние Coupled/Decoupled.
- **Зум:** + / -
...
- Отпускание стика — опциональный автотормоз (если brake_on_release). Кнопка **Brake** (мобайл) эквивалентна Space на ПК: включает ассист плавного торможения до |v|→0 с учётом SR‑клампов и g‑лимитов, **без изменения** состояния Coupled/Decoupled.
...
**Сброс фиксированного Decoupled:** режим, включённый двойным тапом, автоматически сбрасывается при Restart и при смене ориентации. Нажатие Brake/Space не изменяет состояние Coupled/Decoupled и не снимает фиксацию.

 

**Зум на мобайле:** кнопки zoom_plus/zoom_minus изменяют масштаб в пределах render.zoom.min..max (см. AppConfig). Один тап — изменение на **10%** диапазона зума; удержание приводит к авто‑повтору со шагом раз в **100 мс** до достижения пределов. Значения клампятся в [min,max] и отражаются в HUD.
...
Полная спецификация геометрии, осей и соответствия маске спрайта находится в **«U2 — ТЗ ShipConfig v0.5.3» (§3 Geometry)**. В FlightTest фиксируется только правило валидации: tight‑bbox по маске (alpha ≥ alpha_thr) через m_per_px должен совпадать с ship.json/geometry.bbox_m в допустимом допуске.

 

Пример 25.0×20.5 м относится **только к тестовому истребителю** и не переопределяет данные из ship.json других кораблей.
...
**Удаление при (M' ≤ 0):** если после столкновения рассчитанная масса астероида M' = max(M − m_ship, 0) равна нулю или меньше, астероид **удаляется** из мира. Плотность ρ постоянна; каждый раз после изменения массы радиус пересчитывается из объёма: r' = r · (M'/M)^(1/3).
...
**Что именно сбрасывается при Restart:** позиция/скорость/угол/угловая скорость корабля; состояние ассистов (Coupled=ON); режим коллизий = collision.mode_default; видимость оверлея = collision.overlay_visible_default; зум = render.zoom.start.
...
- **FF6:** Автопилот Random: активен по умолчанию; отключается на первый ручной ввод или по клавише R; повторное R включает снова; действует в рамках SR‑клампов и g‑лимитов; не меняет Coupled/Decoupled.
...
- **MF2:** Удержание pad ≥ hold_to_enable_ms — временный Decoupled; двойной тап — фиксация; отпускание стика — Brake (если включён). Нажатие Brake не влияет на режим Coupled/Decoupled.
...
- **Клавиша ****R** относится к **автопилоту**; функции «randomize asteroids/seed++» из дебаг‑конфигов **удалены в v0.5.3** или доступны только из внутренних отладочных инструментов.
...
- **U2 — ТЗ AppConfig v0.5.3**
- **U2 — ТЗ ShipConfig v0.5.3**
